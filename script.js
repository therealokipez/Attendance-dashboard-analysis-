...
function doLogin(role){
  ...
  var isDemo=(em===c.demo.e&&pw===c.demo.p);
  var identity=null; // {name, ini}
  if(isDemo){
    identity={name:c.name,ini:c.ini};
  }else{
    var sk={student:'students',lecturer:'lecturers',advisor:'advisors',hod:'hods',admin:'admins',provost:'provosts'}[role]||'students';
    var acct=ACCOUNT_STORE[sk].filter(function(a){return a.email.toLowerCase()===em.toLowerCase();})[0];
    ...
    if(acct.status==='pending'){
      if(ePw){ePw.textContent='This account is still pending admin approval (usually within 24h)';ePw.classList.add('on');}
      shake();return;
    }
    identity={name:acct.name,ini:acct.name.split(' ').map(function(x){return x[0];}).join('').toUpperCase()};
  }
  ...
  setTimeout(function(){
    ...
    setTimeout(function(){if(b){b.innerHTML=orig;b.disabled=false;}enterDash(role,identity);},900);
  },1600);
}
...
function doRegister(role, ac){
  ...
  setTimeout(function(){
    ...
    var sk={student:'students',lecturer:'lecturers',advisor:'advisors',hod:'hods',admin:'admins',provost:'provosts'}[role]||'students';
    ACCOUNT_STORE[sk].push({id:role.slice(0,3).toUpperCase()+'-'+Math.floor(Math.random()*900+100),name:fn+' '+ln,email:em,password:pw,role:role,dept:'Pending',status:'pending',created:new Date().toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'})});
...
function enterDash(role,identity){
  var c=RC[role];
  var name=(identity&&identity.name)||c.name;
  var ini=(identity&&identity.ini)||c.ini;
  document.getElementById('topAv').textContent=ini;document.getElementById('topAv').style.background=c.avBg;
  document.getElementById('umName').textContent=name;document.getElementById('umRole').textContent=c.role.split('·')[0].trim();
  document.getElementById('sbAv').textContent=ini;document.getElementById('sbName').textContent=name;document.getElementById('sbRole').textContent=c.role;
  ...
  show('s-dash');toast('✅ Welcome, '+name+'!');
}
...
function approveAccount(id){
  var lists=['students','lecturers','advisors','hods','admins','provosts'];
  for(var i=0;i<lists.length;i++){
    var acct=ACCOUNT_STORE[lists[i]].filter(function(a){return a.id===id;})[0];
    if(acct){acct.status='active';toast('✅ '+acct.name+' approved — they can now sign in');renderAcctTable(_acctFilter,'');return;}
  }
}
...
function submitCreateAccount(){
  ...
  ACCOUNT_STORE[sk].push({id:newId,name:fn+' '+ln,email:em,password:pw,role:r,dept:dept,status:'active',created:new Date().toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'})});
